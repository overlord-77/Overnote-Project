...
"""
Overnote version 1.4 on Branch -> Exe <- From Branch -> Main <-
...
class MainApp(ctk.CTk):
    def __init__(self, title, size):
        ...
        icon_path = self.resource_path("D:\\X_OneDrive\\Python\\- Python Scripts - Projects  -\\Overnote\\Overnote - ver.1.4\\assets\\o_icon.ico") #------------------------------> Un-comment once turning it into an exe | re comment it once being updates 
...
class MainOptionsBar(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.tab_system_frame = TabSystemFrame(frame_to_hold, self.main_text_area, self.renderer)
        self.status_bar_frame = StatusBarFrame(frame_to_hold, self.main_text_area, self.format_markdown_bridge)

        self.left_option_bar = LeftOptionsBar(frame_to_hold,  self.status_bar_frame, self.tab_system_frame, self.format_markdown_bridge)
...
class LeftOptionsBar(ctk.CTkFrame):
    def __init__(self, parent, status_bar_frame, tab_system_frame, format_markdown_bridge):
        super().__init__(parent, fg_color= '#202022')
...
class View_Menu(tk.Menu):
    def __init__(self, parent, status_bar_frame, tab_system_frame, file_menu_file_name, format_markdown_bridge, tearoff=False):
        ...
        # Bind Ctrl + Mousewheel # self.bind("<Control-MouseWheel>", lambda e: zoom_in() if e.delta > 0 else zoom_out())
        # # Bind Keyboard Zoom (Ctrl + + / Ctrl + -)
        tab_system_frame.main_text_area.bind("<Control-plus>", zomm_in_f) 
        tab_system_frame.main_text_area.bind("<Control-minus>", zomm_out_f)
        ...
        def markdown_syntax_to_formatted():
                if not self.is_rendered:
                    ...
                    html_body = markdown.markdown(self.original_markdown, extensions=["tables", "fenced_code"])

                    # 1) Make table tag robust (works for <table> and <table ...>) 69b6e2
                    html_body = html_body.replace("<table",'<table border="1" cellpadding="6" cellspacing="0" width="100%" style="border-collapse:collapse;"')
                    # 2) Force borders on header cells
                    html_body = html_body.replace("<th>", '<th style="border:1px solid #69b6e2; padding:6px; text-align:left;">')
                    # 3) Force borders on normal cells
                    html_body = html_body.replace("<td>", '<td style="border:1px solid #69b6e2; padding:6px;">')
                    # link color
                    html_body = html_body.replace("<a ", '<a style="color:#1b87a8; text-decoration:underline;" ')
                    ...
                    <body bgcolor="#121212">
                    <div style="background:#121212; color:#ffffff; padding:12px;font-family: JetBrains Mono, Segoe UI Symbol, Segoe UI Emoji, Arial; padding:12px; a:"red"">
                    {html_body}
                    </div>
                    </body>
                    </html>"""
...
class TabSystemFrame(ctk.CTkFrame):
    def __init__(self, parent, main_text_area, renderer_markdown_text):
        super().__init__(parent)
