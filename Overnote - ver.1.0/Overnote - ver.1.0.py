...
"""
Overnote version 1.0 on Branch -> Exe <- From Branch -> Main <-
...
class MainApp(ctk.CTk):
    def __init__(self, title, size):
        ...
        icon_path = self.resource_path("D:\X_OneDrive\Python\- Python Scripts - Projects  -\Overnote\Overnote - ver.1.0\assets\o_icon.ico") #------------------------------> Un-comment once turning it into an exe | re comment it once being updates 
...
class File_Menu(tk.Menu):
    def __init__(self, parent, tab_system_frame, tearoff=False):
        ...
        self.add_cascade(label='Save', command= save_function)         # FOCUS IS HERE 
...
class TabSystemFrame(ctk.CTkFrame):
    def __init__(self, parent, main_text_area):
        ...
        def new_tab():
            ...
            tabs_data[name] = ""
            tab_button = ctk.CTkButton(tab_bar, text=name, command=lambda n=name: switch_tab(n))
            tab_button.pack(side="left", padx=2)
            switch_tab(name)

            tab_button.bind("<Double-1>", lambda event: rename_box())
        ...
        add_button = ctk.CTkButton(tab_bar, text="+", width=40, command= lambda: new_tab())
...
if __name__ == "__main__":                           #
    MainApp('OverNote', (1400, 750)) # Start the app #
# ---------------------------------------------------#