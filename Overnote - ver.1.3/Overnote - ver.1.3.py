...
"""
Overnote version 1.3 on Branch -> Exe <- From Branch -> Main <-
...
class MainApp(ctk.CTk):
    def __init__(self, title, size):
        ...
        icon_path = self.resource_path("D:/X_OneDrive/Python/- Python Scripts - Projects  -/Overnote/Overnote - ver.1.3/assets/o_icon.ico") #------------------------------> Un-comment once turning it into an exe | re comment it once being updates 
...
class LeftOptionsBar(ctk.CTkFrame):
    def __init__(self, parent, status_bar_frame, tab_system_frame):
        ...
        view_menu = View_Menu(view_menubutton, status_bar_frame, tab_system_frame, file_menu)
...
class CenterOptionsBar(ctk.CTkFrame):
    def __init__(self, parent, main_text_area):
        ...
        def toggle_bold():
            current_font = tkFont.Font(font=main_text_area['font'])
            current_size = current_font.cget("size")
            try:
                start = main_text_area.index("sel.first")
                end   = main_text_area.index("sel.last")
            except tk.TclError:
                return  # No selection → do nothing

            # Configure the bold tag ONCE (not every click)
            main_text_area.tag_config("bold", font=("Arial", current_size, "bold"))

            if is_bold.get():
                bold_text_checkbutton.config(fg="#ff4d6d", relief="flat", selectcolor= '#333333', bg="#202022", activebackground="#202022")
                main_text_area.tag_add("bold", start, end)

            else:
                bold_text_checkbutton.config(fg="#bfbfbf", relief="flat", bg="#202022", activebackground="#202022")
                main_text_area.tag_remove("bold", start, end)
...
class View_Menu(tk.Menu):
    def __init__(self, parent, status_bar_frame, tab_system_frame, file_menu_file_name, tearoff=False):
        super().__init__(
            parent, 
            tearoff=False, 
            activebackground= "#FF5D5D", 
        ...
        def test():
            ...
            name, ext = os.path.splitext(file_name) # print(f'extention: {ext}')
            print(f"""
File Name: {name}
File Ext: {ext}""")
            return
        # self.after(2000, test)
        ...
        # or when i add alink it acutally get added as [Youtube]https://www.youtube.com/)
        ...
        # >>>>>>>>>>>--------------------------------------------------------<<<<<<<<<<<<<<<<<
        text_font = font.Font(family="Didact Gothic Regular", size=14)   # the text widget font
        ...
        self.original_markdown = tab_system_frame.main_text_area.get('1.0', 'end')

        # Matches [Label](URL)
        md_link_pattern = re.compile(r'\[([^\]]+)\]\(([^)]+)\)')    

        def render_markdown_to_text(text_widget, markdown_text):
            """
            Converts Markdown links into clickable links in the Text widget.
            """
            text_widget.delete("1.0", "end")  # Clear previous content

            pos = 0  # Track current position
            for match in md_link_pattern.finditer(markdown_text):
                label, url = match.groups()
                start, end = match.span()

                # Insert normal text before the link
                text_widget.insert("end", markdown_text[pos:start])

                # Insert the link label
                tag_name = f"link_{start}"
                text_widget.insert("end", label, tag_name)

                # Style the link
                text_widget.tag_config(tag_name, foreground="#4fa3ff", underline=True)

                # Bind Ctrl+Click to open URL
                def callback(event, url=url):
                    webbrowser.open(url)
                text_widget.tag_bind(tag_name, "<Control-Button-1>", callback)

                pos = end  # Move past this link

            # Insert remaining text after last link
            text_widget.insert("end", markdown_text[pos:])
    
        def markdown_syntax_to_formatted():
            # Get Markdown from the text widget
            md = tab_system_frame.main_text_area.get('1.0', 'end-1c')

            # Store original Markdown so we can restore later
            global original_markdown
            original_markdown = md

            # Convert Markdown to HTML
            html = markdown.markdown(md)

            # Parse HTML into text (we'll style manually for headings/links)
            soup = BeautifulSoup(html, "html.parser")

            # Clear text widget
            # tab_system_frame.main_text_area.delete('1.0', 'end')
            full_text = tab_system_frame.main_text_area.get("1.0", "end-1c")
            render_markdown_to_text(tab_system_frame.main_text_area, full_text)
            ...
            #     markdown_submenu.entryconfig(0, foreground="red")
            #     markdown_submenu.entryconfig(1, foreground="#85c1ef")
        ...
        def formatted_to_markdown_syntax():
            ...
            if not original_markdown:
                print("No original markdown saved!")
                return

            tab_system_frame.main_text_area.delete('1.0', 'end')
            tab_system_frame.main_text_area.insert('1.0', original_markdown)
        ...
        #     md = tab_system_frame.main_text_area.get('1.0', 'end')

        #     # Store original markdown globally or in object
        #     global original_markdown
        #     original_markdown = md
        ...
        #     tab_system_frame.main_text_area.delete('1.0', 'end')
        #     tab_system_frame.main_text_area.insert('1.0', soup)
        ...
        #     if ext == '.txt':
        #         print('this is a text file')
        #         markdown_submenu.entryconfig(0, foreground="#85c1ef")
        #         markdown_submenu.entryconfig(1, foreground="red")
    ...
    def __init__(self, parent, main_text_area, tearoff=False):
        ...
        def title_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h1", font=("Arial", 32))

                # Apply to selected word
                text.tag_add("h1", start, end)

                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h1", f"{line}.0", f"{line}.end")
        ...
        def subtitle_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h2", font=("Arial", 29))
                ...
                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h2", f"{line}.0", f"{line}.end")
        ...
        def heading_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h3", font=("Arial", 26))
                ...
                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h3", f"{line}.0", f"{line}.end")
        ...
        def subheading_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h4", font=("Arial", 23))

                # Apply to selected word
                text.tag_add("h4", start, end)

                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h4", f"{line}.0", f"{line}.end")
        ...
        def section_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h5", font=("Arial", 20))

                # Apply to selected word
                text.tag_add("h5", start, end)

                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h5", f"{line}.0", f"{line}.end")
        ...
        def subsection_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("h6", font=("Arial", 17))

                # Apply to selected word
                text.tag_add("h6", start, end)

                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("h6", f"{line}.0", f"{line}.end")
        ...
        def body_function():
            try:
                ...
                # Configure the heading tag
                text.tag_config("body_font", font=("Arial", 14))

                # Apply to selected word
                text.tag_add("body_font", start, end)

                # Store the line number for continuation
                line = start.split('.')[0]
                text.tag_add("body_font", f"{line}.0", f"{line}.end")

            except tk.TclError:
                pass

        self.add_cascade(label='Title', font= ('Arial', 32), command=title_function)
        self.add_cascade(label='Subtitle', font= ('Arial', 29), command= subtitle_function)
        self.add_cascade(label='Heading', font= ('Arial', 26), command= heading_function)
        self.add_cascade(label='Subheading', font= ('Arial', 23), command= subheading_function)

        self.add_cascade(label='Section', font= ('Arial', 20), command= section_function)
        self.add_cascade(label='Subsection', font= ('Arial', 17), command= subsection_function)
        self.add_cascade(label='Body', font= ('Arial', 14), command= body_function)
    ...
    def __init__(self, parent, main_text_area, tearoff=False):
        ...
        def bulleted_list():
            ...
            for line in range(start_line, end_line + 1):
                areaoftext = text.get(f'{start_line}.0', 'end')
                test = areaoftext.split('.')[0]
                # data for bulleted list
                line_start_bullet = f"{line}.0"
                line_end_bullet = f"{line}.2"  # first two chars to check for bullet
                first_two_bullet = text.get(line_start_bullet, line_end_bullet)
                # data for numbered list
                line_start_num = f"{line}.0"
                trueline = len(test) + 2
                line_end_num = f"{line}.{trueline}"  # first two chars to check for bullet
                first_three_num = text.get(line_start_num, line_end_num)

                if first_two_bullet == "● ":
                    text.delete(line_start_bullet, line_end_bullet) # Remove bullet
                elif first_three_num == f'{test}. ':
                    remove_numbered_location = text.delete(line_start_num, line_end_num)
                    text.insert(line_start_bullet, "● ")
                else:
                    text.insert(line_start_bullet, "● ") # Add bullet
        ...
        def numbered_list():
            ...
            for line in range(start_line, end_line + 1):
                # data for numbered list
                line_start_num = f"{line}.0"
                line_end_num = f"{line}.3"  # first two chars to check for bullet
                first_three_num = text.get(line_start_num, line_end_num)
                # data for bulleted list
                line_start_bullet = f"{line}.0"
                line_end_bullet = f"{line}.2"  # first two chars to check for bullet
                ...
                if first_three_num == f"{number}. ":
                    # Remove numbered
                    text.delete(line_start_num, line_end_num)
                elif first_two_bullet == '● ':
                    remove_bulleted_location = text.delete(line_start_bullet, line_end_bullet)
                    text.insert(line_start_num, f"{number}. ")
                else:
                    text.insert(line_start_num, f"{number}. ")           # Add numbered
                
                number += 1  # Increment for next line
...
class Link_Tab(ctk.CTkToplevel):
    def __init__(self, parent, main_text_area):
        ...
        ########################################  Below code is in test would be continued in 1.4 V #########################################################
        ...
        def insert_link_func():
            ...
            if len(link_url) >= 1:
                ...
                if len(link_name) < 1:
                    markdown_text = f"[{link_url}]({link_url})"
                else:
        ...
        #     unique_tag = f"web_link_{cursor_pos.replace('.', '_')}"

        #     if len(link_url) >= 1:
        #         insert_button['state'] = 'enabled'

        #         full_markdown_syntax_url = f"[{link_url}]({link_url})"          
        #         full_markdown_syntax_url_name = f"[{link_name}]({link_url})" 
                
        #         if len(link_name) < 1:
        #             text.insert(cursor_pos, full_markdown_syntax_url, unique_tag)
        #             text.tag_config(unique_tag, foreground="#FF4F4F", underline=False)
                    
        #             def open_link(event): # Control + Click = open the URL
        #                 webbrowser.open(link_url_fixed) 
        #             text.tag_bind(unique_tag, "<Control-Button-1>", open_link)

        #         else:
        #             text.insert(cursor_pos, full_markdown_syntax_url_name, unique_tag)
        #             text.tag_config(unique_tag, foreground="#FF4F4F", underline=False)
        #             # Convert the whole text to clickable links
        #             full_text = text.get("1.0", "end-1c")
        #             render_markdown_to_text(text, full_text)

        #             def open_link(event): # Control + Click = open the URL
        #                 webbrowser.open(link_url_fixed)
        #             text.tag_bind(unique_tag, "<Control-Button-1>", open_link)

        #             # - so what i need to do is that whne ever someone add a link with name or without name using hte link Tab
        #             # - it actuall stored them as the following exampel: 
        #             # - linke with a name = [Youtube](https://www.youtube.com/)
        #             # - link without a name = [https://www.youtube.com/](https://www.youtube.com/) - it just re use the link url
        ...
        ########################################  Above code is in test #########################################################

        # thius is the working code Below --------------------------------------------------------------------
        ...
        #         if len(link_name) < 1:
        #             text.insert(cursor_pos, link_url, unique_tag)
        #             text.tag_config(unique_tag, foreground="#FF4F4F", underline=True)
        ...
        #         else:
        #             text.insert(cursor_pos, link_name, unique_tag)
        #             text.tag_config(unique_tag, foreground="#FF4F4F", underline=True)
...
class StatusBarFrame(ctk.CTkFrame):
    def __init__(self, parent, main_text_area):
        ...
        def zoom_percent_change(the_current_size):
            ...
            self.zoom_percentage.configure(text=f'{the_current_size}%')
