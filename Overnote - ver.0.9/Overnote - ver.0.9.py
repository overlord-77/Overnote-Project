...
"""
Overnote version 0.9 on Branch -> Exe <- From Branch -> Main <-
...
class MainApp(ctk.CTk):
    def __init__(self, title, size):
        ...
        icon_path = self.resource_path("D:/X-OneDrive/Python/- Python Scripts - Projects  -/Overnote/Overnote - ver.0.9/assets/o_icon.ico") #------------------------------> Un-comment once turning it into an exe | re comment it once being updates 
...
class MainFrameWidget(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.main_option_bar = MainOptionsBar(self, self.status_bar_frame)
        # self.main_text_area = MainTextArea(self)
        self.tab_system_frame = TabSystemFrame(self)
...
class MainOptionsBar(ctk.CTkFrame):
    def __init__(self, parent, status_bar_frame):
        ...
        self.left_option_bar = LeftOptionsBar(self, status_bar_frame)
        self.center_option_bar = CenterOptionsBar(self)
        self.right_option_bar = RightOptionsBar(self)
...
class LeftOptionsBar(ctk.CTkFrame):
    def __init__(self, parent, status_bar_frame):
        ...
        file_menu = File_Menu(file_menubutton)
...
class CenterOptionsBar(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        def toggle_bold():
            if is_bold.get():
                bold_text_checkbutton.config(
                    fg="#ff4d6d",    # reddish pink
                    selectcolor= '#333333',
        ...
        def toggle_italic():
            if is_italic.get():
                italic_text_checkbutton.config(
                    fg="#ff4d6d",    # reddish pink
                    relief="flat",
                    selectcolor= '#333333',
                    bg="#202022",    # lock dark background
...
class RightOptionsBar(ctk.CTkFrame):
    def __init__(self, parent):
        super().__init__(parent, fg_color= '#202022')
        self.grid(column = 2, row = 0, sticky = 'e')
...
class File_Menu(tk.Menu):
    def __init__(self, parent, tearoff=False):
        ...
        self.add_cascade(label='New Window') # to open a new window |-> command= lambda: MainApp('OverNote', (1400,750))
...
class Setting_Window(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        family_dropdown = ctk.CTkOptionMenu(
            ...
            values=["Century Gothic","Times New Roman", "Calibri"], 
            fg_color= '#444444', 
            button_color= '#444444',
...
class StatusBarFrame(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.pack(expand = False, fill='x', side= 'bottom')
        ...
        cursor_line_and_col_count = ctk.CTkLabel(frame_one_text_count, text= 'Ln 1, Col 1')
        cursor_line_and_col_count.grid(column = 0, row = 0, sticky="w", padx=25)

        characters_count = ctk.CTkLabel(frame_one_text_count, text= '0 chracters')
        characters_count.grid(column = 1, row = 0, sticky="w", padx=1)
...
class TabSystemFrame(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        def new_tab():
            ...
            tabs_data[name] = ""
            button = ctk.CTkButton(tab_bar, text=name, command=lambda n=name: switch_tab(n))
        ...
        add_button = ctk.CTkButton(tab_bar, text="+", width=40, command=new_tab)
